...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void startPreview() {
        ...
        mCamera = Camera.open(CAMERA_ID);// 来获取特定的相机

        Camera.Parameters parameters = mCamera.getParameters();
        Camera.Size size = getBestPreviewSize(1080, 1920, parameters);
        if (size != null) {
            parameters.setPreviewSize(size.width, size.height);//设置预览照片的尺寸
        }

        parameters.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO);//获取当前的焦点模式设置。
        parameters.setPreviewFrameRate(20);//设置预览的帧被接收的速率。这是目标帧速率。实际帧速率取决于驱动程序。
...
//          mCamera.setPreviewCallback(mPreviewCallback);
    ...
    private void initMediaRecorder() {
        ...
        mRecorder.setVideoSize(1080, 1920);  // 设置视频大小
